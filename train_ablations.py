...
from src.model_ablations import *
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ablate', type=str, default='action_embedding',
                        help='name of the module to ablate. following are choices: \n'+ 
                        '1) known_visual 2) unknown_visual 3) video_visual 4) hand_pose 5) hand_bbox '+ 
                        '6) action_embedding 7) function_encoding 8) fusion_module 9*) hand')
    parser.add_argument('--partial_ablation', action='store_true', 
                        help='set true if you want modules to be replaced by linear layers.')
    ...
    valid_ablation_options = ("known_visual", "unknown_visual", "video_visual", "hand_pose", 
                            "hand_bbox", "action_embedding", "function_encoding", "fusion_module", "hand")
    assert args.ablate in valid_ablation_options, 'invalid ablate argument'
    ...
    #                         ])
    if args.ablate == 'hand':
        ablate_list = ['hand_pose', 'hand_bbox']
    else:
        ablate_list = [args.ablate]

    model_saveloc = os.path.join(args.model_folder, 'ablation_hierarchy_discovery_run{}'.format(args.run_num))
    model = VanillaEnd2End_Ablation(frame_feat_shape= (1024, 1), tree_embedding_dim = 4, handpose_dim = 126,
             handbbox_dim = 8, timesteps=time_normalized_dimension, ablate=ablate_list, 
             partial_ablation=args.partial_ablation, device=DEVICE)
